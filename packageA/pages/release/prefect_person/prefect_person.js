// packageA/pages/prefect_person/prefect_person.js
const app = getApp();
let province = [];
let city = [];
let area = [];
...
Page({
  ...
  again() {
    ...
    zhao1 = this.data.audits.licenseImgs;
    this.setData({
      name: this.data.audits.companyName,
      post1: this.data.audits.companyLogoOss,
      zhaos1: this.data.audits.licenseImgsOss,
    ...
    if (this.data.audits.licenseImgsOSS){
      that.setData({
        zhao1:true
      })
    }else{
    ...
    if (this.data.audits.companyImgsOss.length == 3){
      this.setData({
        img_show : true
      })
    }
  ...
  modif_imgs() {
    ...
    let data = {
      companyLogo: this.data.post1_name,
      id: this.data.audits.id,
      isHide: that.data.isHide,
